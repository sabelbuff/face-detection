...
class NeuralNet(object):
    def __init__(self, n_features, n_hidden, n_output, actv='relu', lrate=0.00001, reg=0.0001, epoch=5000):
        self.actv = actv
        self.lrate = lrate
        self.epoch = epoch
        self.reg = reg
    ...
    def test(self, x, y):
        ...
        for i in range(len(y)):
            # print(activation2[])
            # print(y[i])
            if np.argmax(activation2[i]) == np.argmax(y[i]):
                right_class += 1
        # print(right_class)
        accuracy = right_class/len(y)
        return accuracy
